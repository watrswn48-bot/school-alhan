...
s = s.replace("{nav('class2',<Church className=\"w-4 h-4\"/>,'حضور القداس',true)}", "");
...
s = s.replace("{activeNavTab==='class2'&&<FridayLiturgyModule session={session}/>} ", "");
fs.writeFileSync(path, s);
console.log('Removed public liturgy tab and restricted lectures to authorized servants.');

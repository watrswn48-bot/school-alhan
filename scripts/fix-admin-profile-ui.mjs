...
// 1) Apply only small, deterministic changes to the cumulative student profile.
{
...
// 3) Move إضافة المواد into the Admin panel and allow it there through its normal permission.
{
  ...
  s = s.replace("type NavTab = 'class1'|'class2'|'class3'|'class4'|'class5'|'class6'|'class7'|'class8'|'admin';", "type NavTab = 'class1'|'class2'|'class3'|'class4'|'class5'|'class6'|'class7'|'class8'|'admin';");
...
// 4) Put subjects inside Admin. This is deliberately idempotent.
{
  const p = 'src/components/AdminPanelModule.tsx';
  let s = read(p);
  if (!s.includes("import { SubjectsManagementModule } from './SubjectsManagementModule';")) {
...
// 5) Remove only the redundant optional text input from curriculum upload.
// The subject selector populated from إضافة المواد is intentionally untouched.
{
  const p = 'src/components/CurriculaModule.tsx';
  let s = read(p);
  s = s.replace(/\n?\s*<input\s+type="text"\s+value=\{formFileName\}[\s\S]*?placeholder="اسم المادة \(اختياري\)"[\s\S]*?\/>/m, '');
  write(p, s);
}

console.log('Deterministic admin/profile cleanup applied.');

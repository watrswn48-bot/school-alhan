...
export function normalizeRole(role?: ServantRole): ServantRole {
  ...
  const legacy=role as string;
  if(legacy==='family_admin'||legacy==='senior_servant') return 'senior_servant';
  return 'junior_servant';
}
export function permissionsForRole(role:ServantRole):ServantPermissions{const actual=normalizeRole(role);if(actual==='admin')return{...ADMIN_PERMISSIONS};if(actual==='senior_servant')return{...SENIOR_SERVANT_PERMISSIONS};return{...JUNIOR_SERVANT_PERMISSIONS};}
export function normalizeServantPermissions(role:ServantRole|undefined,permissions?:ServantPermissions):ServantPermissions{const actual=normalizeRole(role);const base=permissionsForRole(actual);const incoming=permissions||{};if(actual==='admin')return{...ADMIN_PERMISSIONS};for(const key of ALL_SERVANT_PERMISSIONS)if(incoming[key]!==undefined)base[key]=incoming[key];if(incoming.canRecordAttendance===undefined)base.canRecordAttendance=true;return base;}
...
export function roleLabel(role?:ServantRole):string{const actual=normalizeRole(role);if(actual==='admin')return'أبونا';if(actual==='senior_servant')return'خادم كبير';return'خادم صغير';}

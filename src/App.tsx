...
// Ambient Background Watermark of School Logo
const SchoolBackgroundWatermark: React.FC<{ logo: string }> = ({ logo }) => {
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* Central Large Watermark Emblem */}
...
export default function App() {
  ...
  // Active Main Navigation Tab (الخانات الـ 6 الأساسية + لوحة الإدارة)
  ...
  const handleLoginSuccess = (newSession: UserSession) => {
    ...
    if (newSession.mode === 'student') {
      // Find target student profile for student/parent mode
  ...
  // If NOT logged in, display Authentication & Login Module
  if (!session.isLoggedIn) {
    return <LoginModule onLoginSuccess={handleLoginSuccess} />;
  }

  // Student/Parent Mode view (Read-Only access to personal cumulative profile)
  if (session.mode === 'student') {
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 font-['Tajawal'] pb-12 relative overflow-x-hidden">
        {/* Ambient School Logo Background Watermark */}
        ...
        <div className="relative z-10">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-3xl p-6 shadow-xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-100">
                  بوابة متابعة الطالب وولي الأمر (قراءة فقط)
                </h2>
                <p className="text-xs text-slate-400">
                  عرض سجلات حضور القداسات، المحاضرات، التقييم الـ 5 نجوم، والنتائج الأكاديمية
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 bg-rose-500/20 text-rose-300 hover:bg-rose-500 hover:text-white rounded-xl text-xs font-bold transition-all"
              >
              ...
              {student && (
                <button
                  ...
                  className="px-4 py-2 bg-slate-800 hover:bg-sky-500 hover:text-white text-sky-400 rounded-xl text-xs font-bold transition-all"
                >
                  بطاقة الهوية الذكية
                </button>
              )}
          ...
          <div className="bg-slate-950 p-1.5 rounded-2xl border border-slate-800 flex items-center gap-2 overflow-x-auto shadow-inner">
            <button
              onClick={() => setStudentPortalTab('profile')}
            ...
            >
              ...
              <span>سجلي وتدرجي الشماسي التراكمي</span>
            </button>

            <button
              onClick={() => setStudentPortalTab('curricula')}
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 ${
                studentPortalTab === 'curricula'
                  ? 'bg-amber-500 text-slate-950 shadow-md'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
              }`}
            >
              <BookOpen className="w-4 h-4" />
              <span>مناهج وألحان صفي ({student?.level || 'المناهج'})</span>
            </button>
          </div>
          ...
          {student ? (
            studentPortalTab === 'profile' ? (
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <CumulativeProfileModal
                  student={student}
                  isOpen={true}
                  onClose={handleLogout}
                  onLogout={handleLogout}
                  session={session}
                  onGenerateIDCard={(s) => setSelectedStudentForIDCard(s)}
                />
              </div>
            ) : (
              <CurriculaModule
            ...
            <div className="p-12 text-center text-slate-400">
              لم يتم العثور على سجلات هذا الطالب.
            </div>
          )}
        </main>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-['Tajawal'] pb-16 selection:bg-amber-500 selection:text-slate-950 relative overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* DASHBOARD MODULE NAVIGATION TABS (الخانات الـ 4 الأساسية + الإدارة) */}
        <div className="bg-slate-950/90 border border-slate-800 p-2 rounded-3xl shadow-xl flex items-center justify-between gap-2 overflow-x-auto">
          <div className="flex items-center gap-1.5 min-w-max">
            ...
            <button
              onClick={() => setActiveNavTab('class1')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class1'
            ...
            >
              ...
              <span>الخانة 1: الفصول والطلاب</span>
            </button>

            {/* Class 2: Liturgical Attendance (حضور القداس) */}
            <button
              onClick={() => setActiveNavTab('class2')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class2'
                  ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20 ring-2 ring-amber-400/40'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
            ...
            >
              ...
              <span>الخانة 2: حضور القداس الإلهي</span>
            </button>

            {/* Class 3: Lecture Attendance (حضور المحاضرات) */}
            <button
              onClick={() => setActiveNavTab('class3')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class3'
                  ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20 ring-2 ring-amber-400/40'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
            ...
            >
              <BookOpen className="w-4 h-4" />
              <span>الخانة 3: حضور المحاضرات</span>
            </button>

            {/* Class 4: Analytics (الإحصائيات) */}
            <button
              onClick={() => setActiveNavTab('class4')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class4'
                  ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20 ring-2 ring-amber-400/40'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
            ...
            >
              ...
              <span>الخانة 4: الإحصائيات والمصفوفة</span>
            </button>

            {/* Class 5: Curricula & Hymns (المناهج والألحان) */}
            <button
              onClick={() => setActiveNavTab('class5')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class5'
                  ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20 ring-2 ring-amber-400/40'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
            ...
            >
              <BookOpen className="w-4 h-4" />
              <span>الخانة 5: المناهج ومكتبة الألحان</span>
            </button>

            {/* Class 6: Results & Grades (رصد وإدخال النتائج) */}
            <button
              onClick={() => setActiveNavTab('class6')}
              className={`px-4 py-3 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 ${
                activeNavTab === 'class6'
                  ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20 ring-2 ring-amber-400/40'
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900'
            ...
            >
              ...
              <span>الخانة 6: رصد وإدخال النتائج</span>
            </button>
          </div>
          ...
          {session.role === 'admin' && (
            <button
              ...
              className={`px-4 py-3 rounded-2xl text-xs font-black transition-all flex items-center gap-2 shrink-0 ${
                activeNavTab === 'admin'
        ...
        <div key={dataVersion} className="min-h-[70vh]">
          {activeNavTab === 'class1' && (
            <ClassesAndStudentsModule
              session={session}
              onSelectStudentProfile={(stu) => setSelectedStudentForProfile(stu)}
              onGenerateIDCard={(stu) => setSelectedStudentForIDCard(stu)}
            />
          )}

          {activeNavTab === 'class2' && (
            <LiturgyAttendanceModule session={session} />
          )}

          {activeNavTab === 'class3' && (
            <LectureAttendanceModule session={session} />
          )}
          ...
          {activeNavTab === 'class6' && (
            <ResultsManagementModule session={session} />
          )}

          {activeNavTab === 'admin' && session.role === 'admin' && (
            <AdminPanelModule session={session} />
          )}
        </div>
      </main>

      {/* CUMULATIVE STUDENT PROFILE MODAL */}
      {selectedStudentForProfile && (
        <CumulativeProfileModal
          ...
          session={session}
          onGenerateIDCard={(stu) => setSelectedStudentForIDCard(stu)}
          onTriggerPromotion={(stu) => setSelectedStudentForPromotion(stu)}
        />
      )}

      {/* PROMOTION & PROGRESSION MODAL */}
      <PromotionModal
        student={selectedStudentForPromotion}
        isOpen={!!selectedStudentForPromotion}
        onClose={() => setSelectedStudentForPromotion(null)}
        onPromoteSuccess={(updatedStudent) => {
          setSelectedStudentForProfile(updatedStudent);
        }}
      />

        {/* SMART ID CARD PRINTING MODAL */}
        <SmartIDCardModal
          student={selectedStudentForIDCard}
          isOpen={!!selectedStudentForIDCard}
          onClose={() => setSelectedStudentForIDCard(null)}

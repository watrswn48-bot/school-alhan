...
import { egyptDateString, isEgyptFriday } from '../services/egyptTime';
...
export const FridayLiturgyModule: React.FC<{ session: UserSession }> = ({ session }) => {
  ...
  return <div className="space-y-5">
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
      <div className="p-4 border-b border-slate-800"><b className="text-sm">ملخص حضور الجمعات في السنة الدراسية الحالية</b><div className="text-xs text-slate-500 mt-1">إجمالي الجمعات المحتسبة: {fridays.length} • التاريخ الحالي: {egyptDateString()}</div></div>

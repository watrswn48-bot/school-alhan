...
export const SubjectsManagementModule: React.FC<{ session: UserSession }> = ({ session }) => {
  ...
  const grouped = useMemo(() => items.filter(x => x.levelName === levelName && x.yearName === yearName && x.term === term), [items, levelName, yearName, term]);

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManage) return;
    try {
      await saveChantSubject({ name, levelName, yearName, term, createdBy: session.fullName || 'الإدارة' });
      setName(''); setItems(getChantSubjects()); setMessage('تمت إضافة المادة وربطها بالسنة والترم.');
  ...
  return <div className="space-y-5">
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6">
      ...
      <p className="text-xs text-slate-400 mt-1">المواد تُعرّف مرة واحدة لكل مستوى/سنة/ترم، ثم تظهر تلقائيًا لكل طالب في نفس السنة.</p>
    </div>

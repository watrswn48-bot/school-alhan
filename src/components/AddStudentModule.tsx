  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={submit} className="bg-slate-900 border border-slate-700 rounded-3xl p-6 shadow-2xl space-y-4 text-xs">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2"><label className="label">الصورة الشخصية: <span className="req">*</span></label><div className="relative"><HardDrive className="absolute right-3 top-2.5 w-4 h-4 text-slate-500"/><input required type="text" value={form.photoUrl} onChange={e=>set('photoUrl',e.target.value)} placeholder="رابط صورة مباشر أو كود Google Drive" className="field pr-9"/><p className="text-[10px] text-slate-500 mt-1">الرابط فقط — لا يوجد رفع من الجهاز. لو الصورة على Google Drive اجعل الملف متاحًا لمن لديه الرابط.</p></div>{form.photoUrl && <img src={normalizePhotoSource(form.photoUrl)} alt="معاينة الصورة" className="mt-2 w-16 h-16 rounded-2xl object-cover ring-1 ring-amber-500/40"/>}</div>

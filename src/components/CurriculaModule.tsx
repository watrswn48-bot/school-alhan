...
export const CurriculaModule: React.FC<CurriculaModuleProps> = ({ session, studentLevel, studentYear, studentClass, isStudentPortal = false }) => {
  ...
  const [onlyMyClass, setOnlyMyClass] = useState(isStudentPortal || !!studentLevel || !!studentClass);
  ...
  return <div className="space-y-6 animate-fade-in">
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-2xl bg-amber-500/20 text-amber-400 border border-amber-500/30 flex items-center justify-center"><BookOpen className="w-6 h-6"/></div><div><h2 className="text-xl font-black">مكتبة مناهج الشمامسة والألحان</h2><p className="text-xs text-slate-400 mt-1">{isStudentPortal ? 'المناهج الخاصة بالمستوى والسنة والفصل المحددين' : 'إضافة وإدارة روابط المناهج والمذكرات'}</p></div></div><div className="flex gap-3">{(studentLevel || studentClass || isStudentPortal) && <button onClick={() => setOnlyMyClass(!onlyMyClass)} className={`px-4 py-2.5 rounded-2xl text-xs font-bold border ${onlyMyClass ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-300'}`}><Sparkles className="w-4 h-4 inline ml-2"/>مناهج صفي</button>}{canUpload && !isStudentPortal && <button onClick={() => { setSubjectsList(getChantSubjects()); setIsUploadModalOpen(true); }} className="px-5 py-2.5 bg-amber-500 text-slate-950 font-black text-xs rounded-2xl flex items-center gap-2"><Plus className="w-4 h-4"/>رفع منهج جديد</button>}</div></div>

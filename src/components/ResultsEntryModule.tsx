...
const SCHOOL_CLASSES = ['كيجي', 'أولى وتانية', 'تالتة ورابعة', 'خامسة وسادسة', 'إعدادي وثانوي'];
...
export const ResultsEntryModule: React.FC<{ session: UserSession }> = ({ session }) => {
  ...
  const existing = useMemo(() => {
    ...
    return getSubjectResults(student.id).filter(r =>
      r.levelIndex === student.levelIndex &&
      r.yearIndex === student.yearIndex &&
      r.term === term
    );
    ...
    <div className="space-y-5">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6">
        <h2 className="font-black text-lg flex items-center gap-2">
          <ClipboardCheck className="w-5 h-5 text-amber-400" /> إضافة النتائج
        </h2>
        <p className="text-xs text-slate-400 mt-1">اختار المستوى ثم السنة ثم الفصل، وبعدها اختار كل طالب وسجل نتيجته واحدًا واحدًا.</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-5">
        <div className="flex items-center gap-2 text-sm font-black mb-4">
          <Folder className="w-5 h-5 text-amber-400" /> تحديد مكان الطلاب
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <label className="text-xs font-bold text-slate-300">
            ١. المستوى
            <select value={level} onChange={e => chooseLevel(e.target.value)} className="mt-2 w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-3">
          ...
          <label className="text-xs font-bold text-slate-300">
            ٢. السنة
            <select value={year} onChange={e => chooseYear(e.target.value)} disabled={!level} className="mt-2 w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-3 disabled:opacity-40">
          ...
          <label className="text-xs font-bold text-slate-300">
            ٣. الفصل
            <select value={schoolClass} onChange={e => chooseClass(e.target.value)} disabled={!year} className="mt-2 w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-3 disabled:opacity-40">
              <option value="">اختر الفصل</option>
              {SCHOOL_CLASSES.map(x => <option key={x}>{x}</option>)}
            </select>
          </label>
        </div>

        {(level || year || schoolClass) && (
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
            <span className="px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300">المستوى: {level || '—'}</span>
            <span className="px-3 py-2 rounded-xl bg-sky-500/10 border border-sky-500/20 text-sky-300">السنة: {year || '—'}</span>
            <span className="px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300">الفصل: {schoolClass || '—'}</span>
            <button onClick={resetSelection} className="mr-auto text-slate-400 hover:text-white underline">إعادة الاختيار</button>
          </div>
        )}
      </div>
      ...
      {level && year && schoolClass && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="font-black flex items-center gap-2"><Users className="w-5 h-5 text-amber-400" /> الطلاب</h3>
              <p className="text-xs text-slate-500 mt-1">{matchingStudents.length} طالب في هذا الفصل — اختار طالبًا لإدخال نتيجته.</p>
            </div>
            <label className="text-xs font-bold text-slate-300 sm:w-64">
              الترم
              <select value={term} onChange={e => setTerm(e.target.value as AcademicTerm)} className="mt-2 w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2.5">
          ...
          {!matchingStudents.length ? (
            <div className="p-12 text-center text-slate-500 text-sm">لا يوجد طلاب مسجلون في هذا المستوى والسنة والفصل.</div>
          ) : (
            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {matchingStudents.map((s, index) => (
                <button key={s.id} onClick={() => { setStudentId(s.id); setMessage(''); }} className={`text-right p-4 rounded-2xl border transition-all ${studentId === s.id ? 'bg-amber-500/15 border-amber-500/50' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-amber-400 font-black">{index + 1}</div>
                    <div className="min-w-0">
                      <div className="font-black truncate">{s.fullName}</div>
                      <div className="text-[11px] text-slate-500 mt-1">{s.studentCode}</div>
                    </div>
                    <UserCheck className="w-4 h-4 mr-auto text-slate-500" />
                  </div>
                </button>
              ))}
            </div>
      ...
      {student && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-slate-800 flex items-center gap-3">
            <button onClick={() => setStudentId('')} className="p-2 rounded-xl bg-slate-800 text-slate-300 hover:text-white" title="العودة لقائمة الطلاب">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div>
              <div className="text-xs text-slate-500">إدخال نتيجة الطالب</div>
              <h3 className="font-black text-lg">{student.fullName}</h3>
              <div className="text-xs text-slate-500 mt-1">{level} / {year} / {schoolClass}</div>
            </div>
          </div>

          {!subjects.length ? (
            <div className="p-12 text-center text-slate-500 text-sm">لا توجد مواد مضافة لهذه السنة في {term}. أضف المواد أولًا من خانة «إضافة المواد».</div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[620px] text-sm">
                  <thead className="bg-slate-950 text-slate-400 text-xs">
                    <tr><th className="text-right p-4">المادة</th><th className="p-4">الدرجة من 100</th><th className="p-4">النسبة</th><th className="p-4">الحالة</th></tr>
                  </thead>
                  <tbody>
                    {subjects.map(subject => {
                      ...
                      return (
                        <tr key={subject.id} className="border-t border-slate-800">
                          <td className="p-4 font-bold">{subject.name}</td>
                          <td className="p-4 text-center"><input disabled={!canManage} type="number" min={0} max={100} value={value} onChange={e => setScores(prev => ({ ...prev, [subject.id]: e.target.value }))} className="w-28 max-w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-center" /></td>
                          <td className="p-4 text-center">{filled ? `${Math.max(0, Math.min(100, n))}%` : '—'}</td>
                          <td className={`p-4 text-center font-bold ${!filled ? 'text-slate-500' : n >= 50 ? 'text-emerald-400' : 'text-rose-400'}`}>{!filled ? 'لم تُرصد' : n >= 50 ? 'ناجح' : 'أقل من 50%'}</td>
                        </tr>
                      );
                    })}
                  </tbody>

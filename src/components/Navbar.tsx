      ...
      <div className="max-w-7xl mx-auto w-full px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3">
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 w-full sm:w-auto">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 min-w-0">
                <h1 className="text-base sm:text-2xl lg:text-3xl font-black text-slate-100 tracking-tight truncate">مدرسة تي اتشرومبي للألحان</h1>
              </div>
              <p className="hidden sm:block text-xs sm:text-sm text-slate-400 font-medium truncate">نظام إدارة مدرسة الشمامسة ومتابعة الحضور والخدمة</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto min-w-0">
            ...
            <button onClick={() => setLightMode(v => !v)} className="shrink-0 w-10 h-10 rounded-xl bg-slate-800 text-amber-300 border border-slate-700 hover:bg-amber-500 hover:text-slate-950 transition-colors flex items-center justify-center" title={lightMode ? 'الوضع الغامق' : 'الوضع الفاتح'} aria-label={lightMode ? 'الوضع الغامق' : 'الوضع الفاتح'}>
            ...
            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-2.5 sm:px-3 py-1.5 rounded-2xl min-w-0 max-w-[48vw] sm:max-w-none">
              <div className="p-1 bg-amber-500/20 text-amber-400 rounded-xl shrink-0">{session.role === 'admin' ? <Shield className="w-4 h-4" /> : <UserCheck className="w-4 h-4" />}</div>
              <div className="text-right min-w-0">
                <span className="block text-xs font-bold text-slate-200 max-w-[110px] sm:max-w-[160px] truncate">{session.fullName || 'مستخدم النظام'}</span>
                <span className="hidden sm:block text-[10px] text-amber-400/90 font-medium truncate">{session.mode === 'student' ? 'بوابة ولي الأمر / الطالب' : session.role === 'admin' ? 'مسؤول النظام (Admin)' : 'خادم مدرسة الشمامسة'}</span>
              </div>
            </div>

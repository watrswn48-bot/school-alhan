 ...
 * الشريط العلوي لمنصة مدرسة الألحان
 */
            ...
            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-2.5 sm:px-3 py-1.5 rounded-2xl min-w-0 max-w-[48vw] sm:max-w-none">
              ...
              <div className="text-right min-w-0">
                ...
                <span className="hidden sm:block text-[10px] text-amber-400/90 font-medium truncate">
                  {session.mode === 'student' ? 'بوابة ولي الأمر / الطالب' : session.role === 'admin' ? 'مسؤول النظام (Admin)' : 'خادم مدرسة الشمامسة'}
                </span>
              </div>
            </div>

            <button onClick={onLogout} className="shrink-0 min-h-[42px] px-3 sm:px-4 py-2 bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 hover:text-rose-200 border border-rose-500/30 rounded-xl sm:rounded-2xl text-xs font-black transition-colors cursor-pointer flex items-center justify-center gap-1.5 whitespace-nowrap" title="تسجيل الخروج / تبديل الحساب" aria-label="تسجيل الخروج">
